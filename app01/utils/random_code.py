...
# 生成驗證碼
def random_code(size=(192, 44), length=4, point_num=100, line_num=15):
    width, height = size  # 圖片寬度、高度
    ...
    # 生成用體對象
    font = ImageFont.truetype(font='static/my/font/Clap Hand.ttf', size=24)

    # 書寫文字
    valid_code = ''  # 存放驗證碼字串
    for i in range(length):
        random_char = random.choice(str_all)
        draw.text((45 * i + 20, 10), random_char, (0, 0, 0), font=font)
    ...
    return (data, valid_code)  # 返回 (圖片資料,驗證碼)
